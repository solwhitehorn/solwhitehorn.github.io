...
Every Christmas I gift something to myself and this year I decided to upgrade in the mouse departement. So far I was using a Steelseries Rival 100 that gave me a lot of satisfaction but the cable was short and I really wanted to try a wireless mouse for once. So I got myself the [Logitech MX Master 3](https://www.logitech.com/en-us/products/mice/mx-master-3.html). This is an amazing mouse, a bit pricy at $100 but you got: wireless, 7 buttons and two wheels, button gesture, USB-C and up-to 70 days of use on a full charge. The very interesting thing about this mouse is the support of gestures associated with holding a button. The default feature is using the thumb button and move the mouse up, down, left or right to operate a command. On top of that, you can have gestures working with the forward, back and mode change buttons.

Once you plugin the USB dongle, the mouse will be automatically recognize and all buttons will work but without gestures enabled. In windows you should be using Logitech proprietary software to configure the mouse but in Linux we are going to use this little neet soft: [Logiops](https://github.com/PixlOne/logiops). As always, follow the instruction on the wiki. If you are using Arch, the AUR has all you need.

Once installed, you should have to create the config file yoursself so don't forget to:
```
sudo touch /etc/logid.config
```
You can bind buttons and gestures to specific keys or combinaison of keys. For references you can use the input events code from L. Torvald ([input-event-codes.h](https://github.com/torvalds/linux/blob/master/include/uapi/linux/input-event-codes.h)).

Before sharing my config file, a picture is worth a thousand words so here what my configuration looks like:

[![Mx Master 3](/images/logid/config.png)](/images/logid/config.png)

Before you copy and use my file a few remarks:
- you can use the thumbwheel to control sound volume but I find the thumbwheel too sensitive and I like to use it in different softwars (it will automatically control the volume in mpv or scroll horizontaly in libreoffice for example). I left the code commented in my config file if you are interested.
...
- I'm using KDE so some of the keys are specific to my DE (such as activity switching or opening).
